...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const tableExists = await queryInterface.tableExists('restaurants');
    if (tableExists) {
      console.log('Table restaurants already exists, skipping table creation');
      // Vẫn cần kiểm tra và tạo index nếu chưa có
      const indexes = await queryInterface.showIndex('restaurants');
      const indexNames = indexes.map(idx => idx.name);
      
      if (!indexNames.includes('idx_restaurants_category_id')) {
        await queryInterface.addIndex('restaurants', ['category_id'], {
          name: 'idx_restaurants_category_id'
        });
      }
      if (!indexNames.includes('idx_restaurants_location')) {
        await queryInterface.addIndex('restaurants', ['latitude', 'longitude'], {
          name: 'idx_restaurants_location'
        });
      }
      if (!indexNames.includes('idx_restaurants_status')) {
    ...
    // Tạo index cho category_id để tối ưu query (chỉ nếu chưa tồn tại)
    const indexes = await queryInterface.showIndex('restaurants');
    const indexNames = indexes.map(idx => idx.name);
