...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      // Kiểm tra xem đã có restaurants chưa
      const existingRestaurants = await queryInterface.sequelize.query(
        `SELECT COUNT(*) as count FROM restaurants`,
        { type: queryInterface.sequelize.QueryTypes.SELECT }
      );

      const restaurantCount = existingRestaurants[0].count;

      if (restaurantCount > 0) {
        console.log(`ℹ️  Đã có ${restaurantCount} restaurants trong database, bỏ qua seeding...`);
        return;
      }

      console.log(`   ➜ Đang thêm ${restaurantsWithDates.length} restaurants...`);
      await queryInterface.bulkInsert('restaurants', restaurantsWithDates, {
        ignoreDuplicates: true
      });
      console.log(`✅  Đã thêm ${restaurantsWithDates.length} restaurants thành công`);
    } catch (error) {
      console.error('❌ Lỗi khi seed restaurants:', error.message);
      // Thử insert với ignoreDuplicates nếu gặp lỗi
      await queryInterface.bulkInsert('restaurants', restaurantsWithDates, {
        ignoreDuplicates: true
      });
      console.log('✅ Đã thêm restaurants (bỏ qua duplicates)');
    }
  },
