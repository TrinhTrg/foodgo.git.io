...
export const AuthProvider = ({ children }) => {
  ...
  const requestPasswordReset = async (email) => {
    try {
      ...
      return {
        ...
        message: response.message || 'Mã xác nhận đã được gửi đến email của bạn'
      };
    } catch (error) {
      return {
        success: false,
        message: error.message || 'Gửi mã xác nhận thất bại'
      };
    }
  };
  ...
  const verifyPasswordResetCode = async (email, code) => {
    try {
      ...
      return {
        success: response.success,
        message: response.message || 'Mã xác nhận hợp lệ'
      };
    } catch (error) {
      return {
        success: false,
        message: error.message || 'Mã xác nhận không hợp lệ'
      };
    }
  };

  const checkPasswordMatch = async (email, newPassword) => {
    try {
      const response = await authAPI.checkPasswordMatch(email, newPassword);
      return {
        success: response.success,
      ...
      return {
        success: false,
        isMatch: false,
        message: error.message || 'Lỗi kiểm tra mật khẩu'
      };
    }
  };
